...
const W = 1600, H = 900, ACC = "#ff4d2e";
const rng = (s: number) => () => (s = (s * 16807) % 2147483647) / 2147483647;
const P = (a: number[][]) => a.map((q) => q.map((n) => Math.round(n * 10) / 10).join(",")).join(" ");
const hex = (h: string) => [1, 3, 5].map((i) => parseInt(h.slice(i, i + 2), 16));
const lerpC = (a: string, b: string, t: number) => { const A = hex(a), B = hex(b); return "rgb(" + A.map((v, i) => Math.round(v + (B[i] - v) * t)).join(",") + ")"; };

function lowPoly(seed: number, outline: number[][]): string {
  const r = rng(seed), cols = 10, rows = 15, GW = 480, GH = 800, x0 = -20, y0 = 110;
  const g: number[][][] = [];
  for (let j = 0; j <= rows; j++) { g[j] = [];
    for (let i = 0; i <= cols; i++) {
      const e = i === 0 || i === cols || j === 0 || j === rows;
      g[j][i] = [x0 + (i * GW) / cols + (e ? 0 : (r() - 0.5) * 38), y0 + (j * GH) / rows + (e ? 0 : (r() - 0.5) * 38)];
    } }
  let out = "";
  for (let j = 0; j < rows; j++) for (let i = 0; i < cols; i++) {
    ...
    tris.forEach((t, k) => {
      ...
      let ex = 0; for (let n = 0; n < outline.length - 2; n++) { const [x1, y1] = outline[n], [x2, y2] = outline[n + 1]; if (cy >= y1 && cy <= y2) ex = x1 + (x2 - x1) * ((cy - y1) / (y2 - y1 || 1)); }
      const edge = Math.max(0, 1 - Math.abs(ex - cx) / 130);
      const depth = (cy - 150) / 750;
      const base = lerpC("#2e2e34", "#0c0c0e", Math.min(1, Math.max(0, depth * 0.9 + (r() - 0.5) * 0.35 + k * 0.06)));
      const ember = Math.pow(edge, 1.6) * (1 - depth * 0.55) * (0.55 + r() * 0.45);
      const fill = ember > 0.05 ? lerpC(rgbToHex(base), ember > 0.6 ? "#ff7a52" : ACC, Math.min(0.85, ember)) : base;
      out += `<polygon points="${P(t)}" fill="${fill}" stroke="${fill}" stroke-width="1"/>`;
    });
  }
  return out;
}
function rgbToHex(rgb: string) { return "#" + (rgb.match(/\d+/g) || []).map((n) => (+n).toString(16).padStart(2, "0")).join(""); }

const CL = [[0,160],[120,182],[240,220],[330,288],[368,372],[338,470],[384,560],[348,662],[398,762],[372,900],[0,900]];
const CR = [[0,196],[140,206],[250,248],[335,306],[350,396],[380,478],[346,566],[394,668],[362,900],[0,900]];
const ARCH = "M340 860 C470 250 1130 250 1260 860";
const FAR = "M0 640 L80 560 L140 590 L210 470 L245 500 L290 380 L320 420 L352 300 L372 330 L398 250 L420 360 L470 440 L540 480 L600 380 L640 300 L662 330 L690 200 L712 250 L736 150 L758 230 L790 190 L812 290 L846 350 L900 300 L940 230 L962 270 L990 180 L1016 260 L1050 330 L1110 400 L1170 330 L1210 360 L1250 280 L1276 320 L1320 400 L1400 450 L1470 420 L1600 520 V900 H0Z";
const MID = "M0 720 L110 640 L200 676 L330 600 L450 664 L560 626 L690 690 L800 650 L930 700 L1060 630 L1180 680 L1300 624 L1440 680 L1600 640 V900 H0Z";

function bridgeGeo() {
  const bez = (t: number): [number, number] => { const u = 1 - t; return [u*u*u*340 + 3*u*u*t*470 + 3*u*t*t*1130 + t*t*t*1260, u*u*u*860 + 3*u*u*t*250 + 3*u*t*t*250 + t*t*t*860]; };
  const S = Array.from({ length: 500 }, (_, i) => bez(i / 499));
  const archY = (x: number) => { if (x < 340 || x > 1260) return 860; let b = S[0]; for (const s of S) if (Math.abs(s[0] - x) < Math.abs(b[0] - x)) b = s; return b[1] - 24; };
  const win: string[] = [];
  for (let cx = 232; cx <= 1370; cx += 76) {
    const w = 46, top = 388, bottom = Math.min(archY(cx - w / 2), archY(cx + w / 2)) - 14;
    if (bottom - top < 44) continue;
    win.push(`M${cx - w / 2} ${bottom} V${top + w / 2} A${w / 2} ${w / 2} 0 0 1 ${cx + w / 2} ${top + w / 2} V${bottom} Z`);
  }
  return win;
}
...
export function buildSVG(): string {
  const sr = rng(11);
  let stars = ""; for (let i = 0; i < 90; i++) stars += `<circle cx="${(sr() * W) | 0}" cy="${(sr() * 360) | 0}" r="${(sr() * 1.3 + 0.3).toFixed(2)}" fill="#f5f5f4" opacity="${(sr() * 0.6 + 0.2).toFixed(2)}"/>`;
  let grid = ""; for (let x = 0; x <= W; x += 40) grid += `<line x1="${x}" y1="0" x2="${x}" y2="${H}"/>`; for (let y = 0; y <= H; y += 40) grid += `<line x1="0" y1="${y}" x2="${W}" y2="${y}"/>`;
  let posts = ""; for (let x = 190; x <= 1410; x += 20) posts += `<line x1="${x}" y1="312" x2="${x}" y2="330"/>`;
  const win = bridgeGeo();
  const spandrel = "M200 366 H1400 V860 H1260 C1130 250 470 250 340 860 H200 Z";
  const bp = (d: string, extra = "") => `<path class="bp" pathLength="1" d="${d}" ${extra}/>`;
...
export function bindScene(root: HTMLElement): (p: number) => void {
  const q = (k: string) => root.querySelector(`[data-k="${k}"]`);
  const n = Object.fromEntries(["stars","glow","far","word","mid","grid","solid","blue","notes","deploy","trail","light","live","cl","cr","bridge"].map((k) => [k, q(k)])) as Record<string, Element | null>;
  ...
  const set = (el: Element | null, a: string, v: string | number) => el && el.setAttribute(a, String(v));
  const art = root.querySelector<HTMLElement>("[data-art]");
  return (p: number) => {
    if (art) art.style.transform = `translate3d(0,${mix(14, 0, eIO(seg(p, 0.55, 0.9)))}vh,0)`;
    ...
    set(n.far, "transform", `translate(0,${mix(140, 0, eOut(seg(p, 0, 0.7)))})`);
    set(n.mid, "transform", `translate(0,${mix(110, 0, c)})`);
    { const w = eOut(seg(p, 0.5, 0.95)); set(n.word, "transform", `translate(0,${mix(230, 0, w)})`); set(n.word, "opacity", w); }
    ...
    bps.forEach((el, i) => { const d = cl(draw * 1.25 - (i / bps.length) * 0.25); el.style.strokeDasharray = el.getAttribute("stroke-dasharray") ? "" : "1 1"; if (!el.getAttribute("stroke-dasharray")) el.style.strokeDashoffset = String(1 - d); else el.style.opacity = String(d); });
    ...
    set(n.bridge, "transform", `translate(0,${mix(24, 0, eOut(seg(p, 0.25, 0.7)))})`);
    // deploy
    ...
    set(n.trail, "x", 180); set(n.trail, "width", mix(80, 1160, d));
    set(n.live, "opacity", seg(p, 0.86, 0.9));
    set(n.live, "transform", `translate(0,${mix(8, 0, eOut(seg(p, 0.86, 0.92)))})`);
    // html ui
    ui.forEach((el) => { const s = parseFloat(el.dataset.reveal || "0"); const t = eOut(seg(p, s, s + 0.08)); el.style.opacity = String(t); el.style.transform = `translateY(${mix(16, 0, t)}px)`; });
  };
}
