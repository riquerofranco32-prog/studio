  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-50 overflow-hidden">
      ...
      <motion.div
        ...
        animate={{
          opacity: visible ? 1 : 0,
          scale: cursorText ? 1 : isPointer ? 1.4 : 1,
        }}
        transition={{ duration: 0.15, ease: [0.16, 1, 0.3, 1] }}
        className={`pointer-events-none absolute left-0 top-0 flex items-center justify-center rounded-full transition-colors ${
          cursorText
      ...
      >
        {cursorText && (
          <span className="font-mono text-[10px] font-bold tracking-wider uppercase whitespace-nowrap select-none">
            {cursorText}
          </span>
        )}
      </motion.div>
      ...
      {!cursorText && (
        <motion.div
          ...
          className="pointer-events-none absolute left-0 top-0 h-1.5 w-1.5 rounded-full bg-accent"
        />
      )}
    </div>
  );
